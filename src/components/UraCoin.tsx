...
import '../styles/UraCoin.css';

const UraCoin: React.FC = () => {
  // Utiliser useEffect pour charger l'iframe au montage du composant
  React.useEffect(() => {
    // Créer une iframe pour afficher UraCoin
    const iframe = document.createElement('iframe');
    iframe.src = '/uracoin/index.html';
    iframe.style.width = '100%';
    iframe.style.height = '100%';
    iframe.style.border = 'none';
    
    // Vider le contenu du conteneur et ajouter l'iframe
    const container = document.getElementById('uracoin-container');
    if (container) {
      container.innerHTML = '';
          ...
          script.textContent = `
            ...
            document.querySelectorAll('link[rel="stylesheet"]').forEach(link => {
              if (link.href && link.href.startsWith('/')) {
                link.href = '/uracoin' + link.href;
              }
            });
            document.querySelectorAll('script[src]').forEach(script => {
              if (script.src && script.src.startsWith('/')) {
                script.src = '/uracoin' + script.src;
              }
            });
            document.querySelectorAll('img[src]').forEach(img => {
              if (img.src && img.src.startsWith('/')) {
                img.src = '/uracoin' + img.src;
              }
            });
            
            // Corriger les chemins pour les assets
            const baseUrl = '/uracoin/assets/';
            document.querySelectorAll('link[rel="stylesheet"]').forEach(link => {
  ...
  return (
    <div className="uracoin-wrapper">
      <div id="uracoin-container" className="uracoin-container"></div>
    </div>
  );
};

export default UraCoin;
